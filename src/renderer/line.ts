...
import { ChangedProperties, MTextParagraphAlignment, MTextToken, TokenType } from '@mlightcad/mtext-parser'
...
export class MTextLines extends BaseText {
  ...
  processChar(char: string, geometries: THREE.BufferGeometry[]): void {
    ...
    if (!shape) {
      this._hOffset +=
        this.currentFontSize * this.currentWordSpace * this.currentWidthFactor
      return
    }
  ...
  private calcuateLineParams(newFontHeight?: number) {
    this._currentFontScaleFactor = this.fontManager.getFontScaleFactor(
      this.currentFont
    )
  ...
  private toThreeObject(geometries: THREE.BufferGeometry[]) {
    ...
    if (geometries.length > 0) {
      ...
      const meshGeoms = geometries.filter(
        g => g instanceof THREE.ExtrudeGeometry
      )
      const lineGeoms = geometries.filter(
        g => !(g instanceof THREE.ExtrudeGeometry)
      )
